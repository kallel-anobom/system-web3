...
const CONTRACT_ADDRESS = "0x73862d13BD54b0304d1468BAE20C470A2bF1530B"
...
function getContract() {
    ...
    const web3 = new Web3(window.ethereum);
    if (!from) throw new Error("Carteira não permitida!");
...
export async function getOpenRequests(lastId = 0) {
    ...
    return requests.filter((rq: any) => rq.title !== "");
}

export async function openRequest({ title, description, contact, goal }: any) {
    const contract = getContract();
    return contract.methods.openRequest(title, description, contact, Web3.utils.toWei(goal, "ether")).send();
}

export async function closeRequest(id: any) {
    const contract = getContract();
    return contract.methods.closeRequest(id).send();
}

export async function donate(id: number, donationInBnb: any) {
    const contract = getContract();
